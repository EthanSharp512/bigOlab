...
// ANSWER: No, because it is already O(1) because it is going to stop when count reaches 4. You could refactor to make it look prettier, but that would be the only reason. Micro optimizing would not make it faster on the larger sense of things.
...
// ANSWER: I think they are both O(n) because the regex is the same as the as the letters object and regex would only be better in the best case scenario where it wouln't be saving values of 0 for letters that were not in the input string. But as far the grand scheme of it, as soon as you used all 26 letters in a possibily infinite input string, they would be equally as big.
